...
export default function OrdersPage() {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        const mappedOrders: DisplayOrder[] = userOrders.map(order => ({
          ...
          total: order.total,
          status: order.status === 'delivered' ? t('order.status.delivered') : 
                 order.status === 'preparing' ? t('order.status.preparing') :
                 order.status === 'delivering' ? t('order.status.delivering') :
                 order.status === 'cancelled' ? t('order.status.cancelled') : t('order.status.pending'),
        ...
        toast.error(t('order.errorLoading'));
  ...
  return (
    <div className="min-h-screen bg-[#101828]">
      ...
      <h1 className="text-2xl font-bold mb-6">{t('order.myOrders')}</h1>
      
      {isLoading ? (
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-500 mx-auto mb-4"></div>
          <p className="text-gray-600">{t('order.loadingOrders')}</p>
        </div>
      ) : orders.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          <MdReceipt className="mx-auto text-gray-400" size={64} />
          <h2 className="text-xl font-semibold mt-4 mb-2">{t('order.noOrdersFound')}</h2>
          <p className="text-gray-600 mb-6">{t('order.exploreRestaurants')}</p>
          <Link 
            href="/customer" 
            className="inline-block bg-primary-500 text-white px-6 py-2 rounded-md font-medium hover:bg-primary-600 transition-colors"
          >
            {t('order.exploreRestaurantsButton')}
          </Link>
        </div>
      ) : (
            ...
            <div key={order.id} className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-100">
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-500 mt-1">{t('order.orderNumber', { number: order.id })}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 flex justify-between items-center cursor-pointer" onClick={() => toggleOrderDetails(order.id)}>
                <div className="flex items-center">
                  ...
                  <span className="text-gray-800">
                    {order.items.length} {order.items.length === 1 ? t('order.item') : t('order.items')}
                  </span>
                ...
                <div className="flex items-center">
                  <span className="font-medium mr-2">₪ {order.total.toFixed(2)}</span>
              ...
              {expandedOrder === order.id && (
                <div className="p-4 bg-[#101828] border-t border-gray-100">
                  <div className="mb-4">
                    <h4 className="font-medium mb-2">{t('order.orderItems')}</h4>
                  ...
                  <div className="mb-2">
                    <h4 className="font-medium mb-1">{t('address.deliveryAddress')}</h4>
                    <p className="text-gray-700">{order.address}</p>
                  </div>
                  
                  <div className="flex justify-end mt-4 space-x-3">
                    {(order.status === t('order.status.delivering') || order.status === t('order.status.preparing')) && (
                      <button 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <h2 className="text-xl font-semibold">{t('order.trackDeliveryTitle', { number: trackingOrderId })}</h2>
              <button 
