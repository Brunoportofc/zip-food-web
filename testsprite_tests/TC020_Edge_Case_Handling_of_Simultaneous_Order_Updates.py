...
async def run_test():
    ...
    try:
        ...
        # Click on 'Entrar' button to login as admin user.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/header/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Select restaurant profile, input admin email and password, then click sign in.
        frame = context.pages[-1]
        ...
        # Click on 'Orders' link to view and update order statuses.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/aside/nav/div/ul/li[3]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Open a new tab to simulate delivery driver interface and log in as delivery driver.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/aside/nav/div[3]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        await page.goto('http://localhost:3000/delivery/login', timeout=10000)
        

        # Click on delivery profile, input delivery driver credentials, and sign in to simulate concurrent update.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[2]/div/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('delivery@gmail.com')
        ...
        # Logout current user session to clear session conflict, then retry delivery driver login.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[3]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click delivery profile button, then click sign-in button to attempt delivery driver login.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div/div[2]/div/div/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
