...
async def run_test():
    ...
    try:
        ...
        # Click on restaurant profile to proceed to registration or login
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div/div[2]/div/div/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on create account link to go to restaurant registration page
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[3]/div/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill out registration form with valid restaurant details and submit
        frame = context.pages[-1]
        ...
        # Verify session persistence by checking if user is logged in or can access dashboard without re-login
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[3]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input registered restaurant's username and password and submit login form
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[2]/div/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('admin@gmail.com')
        ...
        # Navigate to restaurant login page to login as restaurant user and verify session persistence
        await page.goto('http://localhost:3000/restaurant/login', timeout=10000)
        

        # Return to homepage and navigate through available UI elements to find restaurant login or dashboard
        await page.goto('http://localhost:3000', timeout=10000)
        

        # Click on 'home.start_now' button to proceed to profile selection and then restaurant login
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/main/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click logout button to logout current session
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div[2]/div/div[3]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # Navigate to profile selection page and verify if restaurant profile is logged in or can be accessed
        await page.goto('http://localhost:3000', timeout=10000)
