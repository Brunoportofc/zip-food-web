...
async def run_test():
    ...
    try:
        ...
        # Select customer profile and input login credentials
        frame = context.pages[-1]
        ...
        # Trigger a new order event as a customer
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/main/div/div[2]/div/div[2]/div/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on the Burger King store to enter and place an order
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/main/div/div[2]/div/div[2]/div/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try clicking on the 'See More' button (index 8) to explore more stores or options to place an order
        frame = context.pages[-1]
